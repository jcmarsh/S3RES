/*
 * The following is copied and modified from Player's Threaded Driver
 * (Original comments truncated... GPL v2 license (same as this repo))
 *
 * This is an attempt to create a driver which launches three
 * redundant threads to provide safety from SEU and possibly non-
 * deterministic software faults.
 */
...
RedundantDriver::RedundantDriver(ConfigFile *cf, int section, bool overwrite_cmds, size_t queue_maxlen) :
...
void RedundantDriver::TestCancel()
{
  int oldstate;
...
void RedundantDriver::StartThread(void)
{
...
void RedundantDriver::StopThread(void)
{
...
void* RedundantDriver::DummyMain(void *devicep)
{
...
void RedundantDriver::DummyMainQuit(void *devicep)
{
  // should never be called without a driver pointer
  assert(devicep);
...
bool RedundantDriver::Wait(double TimeOut)
{
  int oldstate, ret;
  pthread_setcancelstate(PTHREAD_CANCEL_ENABLE,&oldstate);
  pthread_testcancel();
