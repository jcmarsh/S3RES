...
int pipeToBuff(struct vote_pipe* pipe) {
  char temp_buffer[MAX_PIPE_BUFF];
  int read_count;

  read_count = TEMP_FAILURE_RETRY(read(pipe->fd_in, temp_buffer, MAX_PIPE_BUFF));

  if (read_count > 0) {
    if (read_count < MAX_PIPE_BUFF - pipe->buff_count) {
      // Space for the data: copy it to the vote_buff
      int end_index = (pipe->buff_index + pipe->buff_count) % MAX_PIPE_BUFF;
      if (end_index > pipe->buff_index && end_index + read_count > MAX_PIPE_BUFF) {
        // need to wrap back to begining, two copies
        int part_read = read_count - ((end_index + read_count) % MAX_PIPE_BUFF);
        printf("pipeToBuff copy %d bytes from %d to %d\n", part_read, end_index, 0);
        memcpy(&(pipe->buffer[end_index]), &(temp_buffer[0]), part_read);
        printf("pipeToBuff copy %d bytes from %d to %d\n", read_count - part_read, 0, part_read);
        memcpy(&(pipe->buffer[0]), &(temp_buffer[part_read]), read_count - part_read);
      } else {
        // simple case: just copy
        memcpy(&(pipe->buffer[pipe->buff_index + pipe->buff_count]), &(temp_buffer[0]), read_count);
...
int buffToPipe(struct vote_pipe* pipe, int fd_out, int n) {
  ...
  if (pipe->buff_index + n < MAX_PIPE_BUFF) { // simple case: just write
    TEMP_FAILURE_RETRY(write(fd_out, &(pipe->buffer[pipe->buff_index]), n));
  } else {
    // memcpy to buffer, then write out.
    char temp_buffer[n];

    int part_write = MAX_PIPE_BUFF - pipe->buff_index - 1;
    printf("buffToPipe: from buffer %d bytes from %d to %d\n", part_write, pipe->buff_index, 0);
    memcpy(&(temp_buffer[0]), &(pipe->buffer[pipe->buff_index]), part_write);
    printf("buffToPipe: from buffer %d bytes from %d to %d\n", n - part_write, 0, part_write);
...
void fakeToPipe(struct vote_pipe* pipe, int n) {
  pipe->buff_index = (pipe->buff_index + n) % MAX_PIPE_BUFF;
...
int compareBuffs(struct vote_pipe *pipeA, struct vote_pipe *pipeB, int n) {
  ...
  if (pipeA->buff_index + n < MAX_PIPE_BUFF) {
    // simple case
    return memcmp(&(pipeA->buffer[pipeA->buff_index]), &(pipeB->buffer[pipeB->buff_index]), n);
  } else {
    int part_cmp = MAX_PIPE_BUFF - pipeA->buff_index;
