...
# 02 Reactive Tri, other DMR baseline
...
# 03 Artpot Tri, planning and mapper DMR baseline, Filter SMR
...
# 04 Artpot Tri, planning and mapper DMR baseline, Filter SMR
...
for index in `seq 0 $ITARS`; do
	...
	timeout $ANOTHER_TIME python injector.py false "kill -9" >> art_kill_test_tri_injector_wl_$index.txt &
	sleep $ANOTHER_TIME
	ps -eo pid,tid,class,rtprio,ni,pri,psr,pcpu,stat,wchan:14,comm >> art_kill_test_tri_injector_wl_$index.txt
	sleep 40
done
...
for index in `seq 0 $ITARS`; do
	...
	timeout $ANOTHER_TIME python injector.py false "kill -9" >> kill_test_tri_injector_wl_$index.txt &
	sleep $ANOTHER_TIME
	ps -eo pid,tid,class,rtprio,ni,pri,psr,pcpu,stat,wchan:14,comm >> kill_test_tri_injector_wl_$index.txt
	sleep 40
done
...
# 07 All Tri /bin/kill -s 37 tests (simulated sdc)
cp $CONFIG_DIR/all_tri_wl.cfg ./config_plumber.cfg
for index in `seq 0 $ITARS`; do
	...
	timeout $ANOTHER_TIME python injector.py false "/bin/kill -s 37" >> sdc_test_tri_injector_wl_$index.txt &
	sleep $ANOTHER_TIME
	ps -eo pid,tid,class,rtprio,ni,pri,psr,pcpu,stat,wchan:14,comm >> sdc_test_tri_injector_wl_$index.txt
	sleep 40
done
...
# 08 All Tri /bin/kill -s 38 tests (simulated control flow error)
cp $CONFIG_DIR/all_tri_wl.cfg ./config_plumber.cfg
for index in `seq 0 $ITARS`; do
	...
	timeout $ANOTHER_TIME python injector.py false "/bin/kill -s 38" >> freeze_test_tri_injector_wl_$index.txt &
	sleep $ANOTHER_TIME
	ps -eo pid,tid,class,rtprio,ni,pri,psr,pcpu,stat,wchan:14,comm >> freeze_test_tri_injector_wl_$index.txt
	sleep 40
done
...
for index in `seq 0 $ITARS`; do
	...
	timeout $ANOTHER_TIME python injector.py false "./inject_error" >> fault_test_tri_injector_wl_$index.txt &
	sleep $ANOTHER_TIME
	ps -eo pid,tid,class,rtprio,ni,pri,psr,pcpu,stat,wchan:14,comm >> fault_test_tri_injector_wl_$index.txt
	sleep 40
done
